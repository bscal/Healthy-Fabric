package me.bscal.healthy.common.util;
...
public interface ResourceGenerators
{
	interface Item extends ResourceGenerators
	{
		/**
	...
	interface Block extends ResourceGenerators
	{
		/**
		...
		{
			pack.addLootTable(id, JLootTable.loot("minecraft:block")
					.pool(JLootTable.pool()
							.rolls(1)
							.entry(JLootTable.entry().type("minecraft:item").name(id.toString()))
	...
	class Facing implements Block
	{
		...
		{
			this.top = Healthy.MOD_ID + ":block/" + top;
			this.front = Healthy.MOD_ID + ":block/" + front;
			this.side = Healthy.MOD_ID + ":block/" + side;
			this.bottom = Healthy.MOD_ID + ":block/" + bottom;
		}
		...
		{
			String model = ResourceGen.prefixPath(id, "block").toString();
			pack.addBlockState(state(variant().put("facing", "east", JState.model(model).y(90))
					.put("facing", "west", JState.model(model).y(270))
					.put("facing", "north", JState.model(model))
					.put("facing", "south", JState.model(model).y(180))), id);
		}
	...
	class FurnaceLike extends Facing
	{
		...
		public FurnaceLike(String top, String front, String side, String bottom, String top_on,
				String front_on, String side_on, String bottom_on)
		{
			super(top, front, side, bottom);
			this.top_on = Healthy.MOD_ID + ":block/" + top_on;
			this.front_on = Healthy.MOD_ID + ":block/" + front_on;
			this.side_on = Healthy.MOD_ID + ":block/" + side_on;
			this.bottom_on = Healthy.MOD_ID + ":block/" + bottom_on;
		}
		...
		public void generateBlockState(RuntimeResourcePack pack, Identifier id)
		{
			String model = ResourceGen.prefixPath(id, "block").toString();
			String on = model + "_on";
			pack.addBlockState(state(variant().put("facing=east,lit=false", JState.model(model).y(90))
					.put("facing=west,lit=false", JState.model(model).y(270))
					.put("facing=north,lit=false", JState.model(model))
					.put("facing=south,lit=false", JState.model(model).y(180))
					.put("facing=east,lit=true", JState.model(on).y(90))
					.put("facing=west,lit=true", JState.model(on).y(270))
					.put("facing=north,lit=true", JState.model(on))
					.put("facing=south,lit=true", JState.model(on).y(180))), id);
		}
